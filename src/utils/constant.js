export const MENU_ICON =
  "https://img.icons8.com/?size=100&id=82749&format=png&color=FFFFFF";

export const YOUTUBE_LOGO =
  "data:image/png;base64,";

export const YOUTUBE_SUGGESTION_API =
  "http://suggestqueries.google.com/complete/search?client=firefox&client=chrome&ds=yt&q=";

// icons
export const USER_ICON =
  "https://img.icons8.com/?size=100&id=7819&format=png&color=FFFFFF";
export const SEARCH_ICON =
  "https://img.icons8.com/?size=50&id=lwZinoeNcL3F&format=png&color=FFFFFF";

export const YOUTUBE_POPULAR_VIDEO_API =
  "https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&maxResults=50&regionCode=US&key=";

const commentjokes = [
  "I'm just energy-saving today.",
  "Why wake up early? Why?",
  "Potato is my spirit animal.",
  "I see food, eat.",
  "Running on caffeine and hope.",
  "Procrastination is my superpower.",
  "I laugh, then cry.",
  "Normal is overrated, obviously.",
  "I’m allergic to responsibilities.",
  "Adulting is not for me.",
  "Mondays need to be banned.",
  "I nap, therefore I exist.",
  "I overthink everything, help.",
  "Why stress? Eat ice cream.",
  "Oops, I broke the internet.",
  "Coffee owns my soul now.",
  "I told you it’s broken.",
  "Wi-Fi is my true friend.",
  "Who stole my last donut?",
  "Sarcasm is my love language.",
  "Laughing at my own jokes.",
  "I can't find my brain.",
  "Why walk? I’ll just crawl.",
  "Oops, I hit snooze again.",
  "I’m not lazy, efficient instead.",
  "Trying hard not to try.",
  "Pizza solves all my problems.",
  "I need unlimited nap time.",
  "Why work? Netflix is calling.",
  "Diets? Not in this lifetime.",
  "I’m just too fabulous, obviously.",
  "I survived without Wi-Fi yesterday.",
  "When’s the next vacation, please?",
  "I’m fluent in sarcasm, thanks.",
  "Weekends are my survival kit.",
  "Why run? Walking’s exhausting enough.",
  "Memes keep me alive, period.",
  "Where’s the pause button, life?",
  "Oops, forgot to adult today.",
  "Haters gonna hate. I sleep.",
  "My brain went on strike.",
  "Why bother? It’s overrated anyway.",
  "Shhh, I’m hiding from reality.",
  "Chocolate makes everything better. Always.",
  "I Googled myself, I’m awesome.",
  "Woke up tired, slept tired.",
  "I’m allergic to serious conversations.",
  "I deserve unlimited ice cream.",
  "I make awkward look cool.",
  "Why me? Oh, right, karma."
];

const commentNames = [
  "Aryan Sharma",
  "Aarav Gupta",
  "Ananya Iyer",
  "Ishita Verma",
  "Karthik Reddy",
  "Meera Nair",
  "Rajiv Choudhary",
  "Nisha Singh",
  "Aditya Rao",
  "Pooja Patel",
  "Rohan Mehta",
  "Sneha Desai",
  "Harsha Menon",
  "Priya Joshi",
  "Vikram Kapoor",
  "Divya Malhotra",
  "Rahul Khanna",
  "Riya Bhattacharya",
  "Siddharth Jain",
  "Krishna Ghosh",
  "Neha Mahajan",
  "Akash Pillai",
  "Anjali Das",
  "Manoj Kulkarni",
  "Tara Agarwal",
  "Shivani Shetty",
  "Ravi Bansal",
  "Deepa Chatterjee",
  "Abhay Nambiar",
  "Swati Mishra",
  "Arjun Yadav",
  "Lakshmi Ramaswamy",
  "Kabir Arora",
  "Pallavi Saxena",
  "Amitabh Dubey",
  "Sunita Kaur",
  "Vivek Tiwari",
  "Kavya Rao",
  "Devika Sharma",
  "Nikhil Deshmukh",
  "Leela Chakraborty",
  "Suresh Naidu",
  "Maya Sen",
  "Chitra Shukla",
  "Aakash Bhatia",
  "Jaya Srinivasan",
  "Sameer Kohli",
  "Meghna Tripathi",
  "Santosh Pawar",
  "Radhika Shinde"
];

export const getRandomFunnyComment = () =>
  commentjokes[Math.floor(Math.random() * commentjokes.length)];
export const getRandomName = () =>
  commentNames[Math.floor(Math.random() * commentNames.length)];


